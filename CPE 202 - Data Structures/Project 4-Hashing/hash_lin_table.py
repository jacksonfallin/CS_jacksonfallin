...
class HashTableLinPr:
    ...
    def read_stop(self, filename):                                    # sift through stop words and hash
        with open(filename, 'r') as fin:                              # with function to open file
            while True:                                               # continue to loop
                stop = fin.readline()
                if stop:
                    if stop.endswith("\n"):                           # go through until stop word ends
                        stop = stop[:-1]
    ...
    def check_helper(self, key):                                      # see if key is there
        i = self.myhash(key, self.table_size)                         # initialize index
        while self.hash[i] is not None and self.hash[i][0] != key:    # if exists
            i %= self.table_size                                      # remain
            i += 1                                                    # increment index
    ...
    def increase_table(self):                                         # increase if table size is reaching max
        ...
        increasehash = HashTableLinPr(self.table_size * 2 + 1)        # double hash size
